...
// ─── Financing Modal ──────────────────────────────────────────────────────────
function FinancingModal({ preco, titulo, onClose }: { preco: number; titulo: string; onClose: () => void }) {
  const [entrada, setEntrada] = useState(20);
  const [meses, setMeses] = useState(60);

  const parcela = calcParcela(preco, entrada, meses);
  const valorEntrada = Math.round(preco * entrada / 100);
  const valorFinanciado = preco - valorEntrada;

  const handleWA = () => {
    const msg = `Olá! Tenho interesse em financiar o ${titulo} por ${fmt(preco)}. Entrada de ${fmt(valorEntrada)} (${entrada}%) e prazo de ${meses}x. Parcela estimada: ${fmt(parcela)}/mês. Gostaria de mais informações!`;
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center p-0 sm:p-4">
      ...
      <motion.div
        ...
        className="relative bg-white rounded-t-2xl sm:rounded-2xl w-full sm:max-w-md shadow-2xl z-10"
      >
        {/* Header */}
        <div className="flex items-center justify-between px-6 pt-5 pb-4 border-b border-atria-gray-medium">
          <div>
            <p className="font-inter text-xs text-atria-text-gray uppercase tracking-wider">Simulação de financiamento</p>
            <p className="font-barlow-condensed font-bold text-lg text-atria-text-dark leading-tight">em parceria com Credere</p>
          </div>
          <button onClick={onClose} className="w-8 h-8 rounded-full bg-atria-gray-light flex items-center justify-center hover:bg-atria-gray-medium transition-colors">
            <X size={16} />
          </button>
        </div>
        ...
        <div className="px-6 py-5 space-y-5">
          {/* Vehicle price */}
          <div className="bg-atria-gray-light rounded-xl p-4 flex items-center justify-between">
            <span className="font-inter text-sm text-atria-text-gray">Valor do veículo</span>
            <span className="font-barlow-condensed font-black text-xl text-atria-navy">{fmt(preco)}</span>
          </div>

          {/* Entrada slider */}
          <div>
            <div className="flex items-center justify-between mb-2">
              <label className="font-inter text-sm font-semibold text-atria-text-dark">Entrada</label>
              <span className="font-inter text-sm font-bold text-atria-navy">{fmt(valorEntrada)} ({entrada}%)</span>
            </div>
            <input
              type="range" min={10} max={70} step={5} value={entrada}
              onChange={(e) => setEntrada(Number(e.target.value))}
              className="w-full accent-atria-navy h-1.5 rounded-full"
              aria-label="Percentual de entrada"
            />
            <div className="flex justify-between mt-1">
              <span className="font-inter text-xs text-atria-text-gray">10%</span>
              <span className="font-inter text-xs text-atria-text-gray">70%</span>
            </div>
          </div>

          {/* Prazo */}
          <div>
            <label className="font-inter text-sm font-semibold text-atria-text-dark block mb-2">Prazo</label>
            <div className="grid grid-cols-5 gap-1.5">
              {[12, 24, 36, 48, 60].map((m) => (
                <button
                  ...
                  type="button"
                  onClick={() => setMeses(m)}
                  className={`py-2 rounded-lg font-inter text-sm font-semibold border-2 transition-all ${
                    meses === m
          ...
          {/* Result */}
          <div className="bg-atria-navy rounded-xl p-4 text-white text-center">
            <p className="font-inter text-sm opacity-80 mb-0.5">Parcela estimada</p>
            <p className="font-barlow-condensed font-black text-4xl">{fmt(parcela)}<span className="text-lg font-inter font-normal opacity-70">/mês</span></p>
            <p className="font-inter text-xs opacity-60 mt-1">Taxa de referência: 1,5% a.m. · {meses}x · Financiado: {fmt(valorFinanciado)}</p>
          </div>

          <p className="font-inter text-xs text-atria-text-gray text-center">
            Simulação sem caráter contratual. Sujeito à análise de crédito.
          </p>

          <button
            onClick={handleWA}
            className="w-full bg-green-500 hover:bg-green-600 text-white font-inter font-bold text-sm uppercase tracking-wider py-3.5 rounded-xl transition-colors flex items-center justify-center gap-2"
          >
    ...
    <>
      <div className="bg-white border border-atria-gray-medium rounded-2xl shadow-sm overflow-hidden">
        <div className="p-6 space-y-4">
          ...
          {/* Credere simulator button */}
          <button
            onClick={() => setFinancingOpen(true)}
            className="w-full border-2 border-atria-navy text-atria-navy font-inter font-semibold text-sm py-3 rounded-xl hover:bg-atria-navy hover:text-white transition-all flex items-center justify-center gap-2"
          >
