                      ...
                      >
                        {(() => {
                          ...
                          if (car.fotos && typeof car.fotos === 'string') {
                            imagesToRender = car.fotos.split(';').filter(url => url.trim());
                          } else if (car.images && Array.isArray(car.images)) {
                            imagesToRender = car.images;
                          } else if (car.imagens && Array.isArray(car.imagens)) {
                            imagesToRender = car.imagens;
                          } else {
                            imagesToRender = [car.imagem_capa || '/images/resource/car1-1.jpg'];
                          }
                          
                          return imagesToRender.map((image, i) => (
                            <div key={i} className="image d-block">
                              <Link to={`/veiculo-individual/${car.id}`}>
                                <img fetchpriority="low" decoding="async" alt={`Foto ${i + 1}`} src={image} width={329} height={220} loading="lazy" />
                              </Link>
                            </div>
                          ));
                        })()}
